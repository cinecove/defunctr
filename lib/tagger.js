...
const extractDocElement = document => (
  (document && document.documentElement && document.documentElement.className !== undefined) ? document.documentElement : { className: '' }
);

const hasTag = (docElement, tag) => (
  docElement.className.length === 0 ? false : docElement.className.match(new RegExp(`\\b${tag}\\b`))
);
...
const test = (docElement) => (key, test) => {
  if (test()) {
...
const tagPage = (detective, document, prefix = '') => {
  const docElement = extractDocElement(document);
  const tester = test(docElement);
